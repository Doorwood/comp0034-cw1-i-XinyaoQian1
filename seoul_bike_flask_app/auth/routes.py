...
def signin():
    ...
    print('signin',"login_form.validate_on_submit(): ",login_form.validate_on_submit())
...
def post_id(post_id) :
    ...
    obj = UserBlogStars.query.filter_by(uid=current_user.id, bid=post_id).first()
    if obj:
    ...
    if star_index :
        ...
        if obj :
            ...
            ubg = UserBlogStars(uid=current_user.id,bid=post_id,count=star_index)
            db.session.add(ubg)
        db.session.commit()
